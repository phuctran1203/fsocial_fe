					...
					<div className="flex items-center gap-2 text-[--gray-clr]">
						...
						<button className="flex items-center" onClick={handleClickLike}>
							<svg className="sm:h-[15px] h-[13px]" width="25" height="22" viewBox="0 0 25 22" fill="none">
								<path
									className={
										like ? "fill-[--primary-clr] stroke-[--primary-clr]" : "fill-[--gray-clr] stroke-[--gray-clr]"
									}
								...
								<path
									className={like ? "fill-[--primary-clr] stroke-[--primary-clr]" : ""}
							...
							<span className={`fs-sm ${like && "text-primary"}`}>{countLikes}</span>
						</button>
...
export default function CommentModal({ id }) {
	...
	const selectCommentToReply = (props) => {
		...
		const exist = Array.from(textbox.current.childNodes).find((el) => el.dataset?.mention === userId);
		if (exist) {
			console.log("đã có trong danh sách nhắc đến");
			return;
		}
		setSelectReply(props);
		console.log("cmt id: ", id, "userId: ", userId, "displayName: ", displayName);
