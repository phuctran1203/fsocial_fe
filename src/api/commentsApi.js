...
export async function likeComment(sendingData) {
  try {
    const resp = await API.post(`/post/comment/like`, sendingData);
    const data = resp.data;
    console.log("Resp likeComment: ", data);
    return data;
  } catch (error) {
    if (error.name === "CanceledError") return null;
    console.error("Error at likeComment: ", error);
